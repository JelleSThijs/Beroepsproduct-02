...
import com.jsthijs.beroepsproduct02.Database;
...
public class HomeScreen implements Screen{
    ...
    private Pane itemList(String type, Integer limit) {
        ...
        Database db = new Database("localhost", "root","beroepsproduct-02");
        String querry = "SELECT items.*, tags.name as tagName FROM items\n" +
        ...
        try {
            ResultSet item = db.executeQuery(querry);
            while(item.next()){
    ...
    private Pane renderItem(Item item, String tagName) {
        ...
        FlowPane release_year = new FlowPane(new Text(item.getRelease_year().toString()));
